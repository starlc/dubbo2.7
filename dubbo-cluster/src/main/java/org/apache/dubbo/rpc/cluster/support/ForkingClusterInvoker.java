 ...
 * ForkingClusterInvoker 中会维护一个线程池（executor 字段，
 * 通过 Executors.newCachedThreadPool() 方法创建的线程池），并发调用多个 Provider 节点，
 * 只要有一个 Provider 节点成功返回了结果，ForkingClusterInvoker 的 doInvoke() 方法就会立即结束运行。
 *
...
public class ForkingClusterInvoker<T> extends AbstractClusterInvoker<T> {
    ...
    /**
     * ForkingClusterInvoker.doInvoke() 方法首先从 Invoker 集合中选出指定个数（forks 参数决定）的 Invoker 对象，
     * 然后通过 executor 线程池并发调用这些 Invoker，并将请求结果存储在 ref 阻塞队列中，则当前线程会阻塞在 ref 队列上，
     * 等待第一个请求结果返回。
     * @param invocation
     * @param invokers
     * @param loadbalance
